...
class PaymentController extends Controller
{
    ...
    public function index(Request $request): JsonResponse|\Inertia\Response
    {
        ...
        $stats = $this->paymentIndexStats($statsBase);
     ...
     */
    protected function paymentIndexStats($query): array
    {
        return [
            'total' => (clone $query)->count(),
            'manual' => (clone $query)->where('processor', 'manual')->count(),
            'stripe' => (clone $query)->where('processor', 'stripe')->count(),
        ];
    }
