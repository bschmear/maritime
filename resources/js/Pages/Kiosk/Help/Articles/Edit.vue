...
/**
 * Laravel sends published_at as ISO 8601. Browsers only accept datetime-local as local YYYY-MM-DDTHH:mm.
 */
...
const form = useForm({
    ...
    article_type: props.article.article_type || 'guide',
    sort_order: props.article.sort_order ?? 0,
    featured: props.article.featured ?? false,
    active: props.article.active ?? true,
    published_at: toDateTimeLocalValue(props.article.published_at),
});

const submit = () => {
    form.put(route('kiosk.help-articles.update', props.article.id));
};
    ...
    <KioskLayout>
        <template #header>
            <div class="flex items-center gap-x-3">
                ...
                <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Article</h1>
            </div>
        </template>

        <form class="mx-auto max-w-7xl" @submit.prevent="submit">
            <p class="mb-6 text-sm text-gray-600 dark:text-gray-400">
                Edit content and article order on the left; use the sidebar for video, category, and publishing.
            </p>
                    ...
                    >
                        ...
                        <div class="px-4 py-6 sm:px-6">
                            <HelpArticleCategoryOrder
                                ...
                                :saved-category-id="article.category_id"
                                :article-id="article.id"
                                :article-title="form.title"
                                :initial-articles="categoryArticles"
