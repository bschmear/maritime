...
const resolveEnumId = (enumKey, value) => {
    ...
    if (typeof value === 'string') {
        const opt = opts.find(o => o.value === value || String(o.value) === value);
        return opt ? opt.id : value;
    }
    return value;
};

const resolveEnumStoredValue = (enumKey, id) => {
    if (id == null || id === '') return id;
    const opts = props.enumOptions?.[enumKey] ?? [];
    const opt = opts.find(
        (o) => String(o.id) === String(id) || o.value === id || String(o.value) === String(id),
    );
    return opt?.value ?? id;
};
...
const form = useForm({
    ...
    payment_term:           resolveEnumId(PAYMENT_TERM_ENUM_KEY, props.record?.payment_term ?? props.initialData?.payment_term ?? 'due_on_receipt'),
...
const selectedPaymentTermOption = computed(() =>
    paymentTermOptions.value.find(
        (o) =>
            String(o.id) === String(form.payment_term)
            || o.value === form.payment_term
            || String(o.value) === String(form.payment_term),
    ),
);

const paymentTermDescription = computed(
    () =>
        selectedPaymentTermOption.value?.description
        || props.fieldsSchema?.payment_term?.description
        || '',
);
...
const submitLabel = computed(() => (props.mode === 'edit' ? 'Save changes' : 'Create contract'));
...
const submit = () => {
    ...
    const transformPayload = (data) => ({
        ...data,
        payment_term: resolveEnumStoredValue(PAYMENT_TERM_ENUM_KEY, data.payment_term),
    });

    if (props.mode === 'edit') {
        form.transform(transformPayload).put(route('contracts.update', props.record.id), validationSubmitOptions({
            onSuccess: () => emit('saved'),
        }));
    } else {
        form.transform(transformPayload).post(route('contracts.store'), validationSubmitOptions({
            onSuccess: () => emit('saved'),
        }));
    }
};
...
<template>
    <div class="w-full flex flex-col space-y-6">
        <form id="contract-form" :class="!isView ? 'pb-28' : ''" @submit.prevent="submit">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-lg sm:rounded-lg overflow-hidden">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <!-- Payment term -->
                            <div>
                                <label for="payment_term" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                    {{ fieldsSchema?.payment_term?.label || 'Payment term' }}
                                </label>
                                <EnumButtonGroup
                                    id="payment_term"
                                    ...
                                    :disabled="isView"
                                />
                                <p v-if="paymentTermDescription" class="mt-2 text-xs text-gray-500 dark:text-gray-400">
