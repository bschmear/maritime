...
const selectedSubsidiaryLabel = ref('');
...
// ── Work order auto-fill watcher ───────────────────────────────────────────
let isInitialLoad = true;
watch(() => form.work_order_id, async (newWorkOrderId, oldWorkOrderId) => {
    // Skip if this is the initial load or if work order hasn't actually changed
    if (isInitialLoad || newWorkOrderId === oldWorkOrderId) {
        return;
    }

    if (!newWorkOrderId) {
        // Work order was cleared, reset related fields
    ...
    try {
        ...
        const { data } = await axios.get(route('deliveries.work-order-details', newWorkOrderId));

        // Auto-fill fields from work order
        form.customer_id = data.customer_id;
        form.asset_unit_id = data.asset_unit_id;
        form.subsidiary_id = data.subsidiary_id;
        form.location_id = data.location_id;

        // Update labels
        selectedCustomerLabel.value = data.customer_name ?? '';
        selectedAssetLabel.value = data.asset_name ?? '';
        selectedSubsidiaryLabel.value = data.subsidiary_name ?? '';
        selectedLocationLabel.value = data.location_name ?? '';
...
const submit = async () => {
    ...
    form.clearErrors();
    try {
        const { data } = await axios.post(route('deliveries.store'), form.data(), {
            headers: { 'X-Modal-Request': '1' },
        });
        emit('saved', data.recordId);
    } catch (err) {
        if (err.response?.status === 422) {
            ...
            form.setError('general', err.response?.data?.message || 'Something went wrong.');
        }
    } finally {
...
const showMainForm = computed(() => stepComplete.value || form.work_order_id || form.customer_id || form.asset_unit_id);
</script>
...
<template>
    <div class="space-y-5">
        ...
        <div v-if="showStep0">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <button type="button" @click="hasWorkOrder = true"
                    class="group relative flex flex-col items-start gap-4 p-5 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-400 hover:shadow-md transition-all text-left">
                    <div class="h-10 w-10 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center group-hover:bg-blue-200 dark:group-hover:bg-blue-900/50 transition-colors">
                        <span class="material-icons text-xl text-blue-600 dark:text-blue-400">assignment</span>
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">Link to Work Order</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
                            Customer, asset, and address will be pre-filled.
                        </p>
                    </div>
                ...
                <button type="button" @click="hasWorkOrder = false"
                    class="group relative flex flex-col items-start gap-4 p-5 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 hover:shadow-md transition-all text-left">
                    <div class="h-10 w-10 rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center group-hover:bg-gray-200 dark:group-hover:bg-gray-600 transition-colors">
                        <span class="material-icons text-xl text-gray-600 dark:text-gray-400">person</span>
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">No Work Order</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
                            Choose a customer and vessel directly.
                        </p>
                    </div>
        ...
        <div v-if="showStepCust">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Select a customer, then choose the vessel</p>
                </div>
            </div>
        ...
        <form v-if="showMainForm" @submit.prevent="submit" class="space-y-5">

            <!-- Confirmation banner -->
            <div class="rounded-xl border border-blue-200 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-900/10 px-4 py-3">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0 flex flex-col sm:flex-row gap-3">
                        ...
                        <div v-if="selectedSubsidiaryLabel" class="min-w-0">
                            <p class="text-xs font-semibold text-blue-500 dark:text-blue-400 uppercase tracking-wide">Subsidiary</p>
                            <p class="text-sm font-semibold text-blue-900 dark:text-blue-200 truncate">{{ selectedSubsidiaryLabel }}</p>
                        </div>
                        <div v-if="selectedLocationLabel" class="min-w-0">
                            <p class="text-xs font-semibold text-blue-500 dark:text-blue-400 uppercase tracking-wide">Location</p>
                            <p class="text-sm font-semibold text-blue-900 dark:text-blue-200 truncate">{{ selectedLocationLabel }}</p>
                        </div>
                    </div>
                    <button type="button" @click="resetFlow"
            ...
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="flex items-center gap-2 px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                    <span class="material-icons text-gray-500 dark:text-gray-400 text-base">calendar_today</span>
            ...
            <!-- ── Delivery Address ────────────────────────────────── -->
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
